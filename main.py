...
def v1_nicolive():
  ...
  if nicolive_last_fetched is None or nicolive_interval <= now - nicolive_last_fetched:
    nicolive_last_fetched_string = nicolive_last_fetched.isoformat() if nicolive_last_fetched is not None else 'None'
    print(f'[{now.isoformat()}] Fetch nicolive (last_fetched_at: {nicolive_last_fetched_string})')
